...
TEST_F(IsometryTests, DefaultConstructor) {
  Isometry uut;
  ASSERT_TRUE(
      areEqualWithinTolerance(uut.rotation(), Matrix3::Identity, tolerance_));
  ASSERT_TRUE(
      areEqualWithinTolerance(uut.translation(), Vector3::Zero, tolerance_));
}
...
TEST_F(IsometryTests, RotTranConstructor) {
  ...
  ASSERT_TRUE(
      areEqualWithinTolerance(uut.translation(), translation, tolerance_));
}

TEST_F(IsometryTests, StaticConstants) {
  auto uut = Isometry::Identity;
  ASSERT_TRUE(
      areEqualWithinTolerance(Matrix3::Identity, uut.rotation(), tolerance_));
  ASSERT_TRUE(
      areEqualWithinTolerance(Vector3::Zero, uut.translation(), tolerance_));
}
...
TEST_F(IsometryTests, Translation) {
  {
    ...
    ASSERT_TRUE(
        areEqualWithinTolerance(translation, uut.translation(), tolerance_));
  }
  {
    // non-const version
    ...
    ASSERT_TRUE(
        areEqualWithinTolerance(translation1, uut.translation(), tolerance_));
    uut.translation() = translation2;
    ASSERT_FALSE(
        areEqualWithinTolerance(translation1, uut.translation(), tolerance_));
    ASSERT_TRUE(
        areEqualWithinTolerance(translation2, uut.translation(), tolerance_));
  }
}
...
TEST_F(IsometryTests, Rotation) {
  {
    // const version
    const Matrix3 rotation1{Vector3{0, 1, 0}, Vector3{0, 0, 1},
                            Vector3{1, 0, 0}};
  ...
  }
  {
    ...
    const Matrix3 rotation1{Vector3{0, 1, 0}, Vector3{0, 0, 1},
                            Vector3{1, 0, 0}};
    const Matrix3 rotation2{Vector3{0, 0, 1}, Vector3{1, 0, 0},
                            Vector3{0, 1, 0}};
    Isometry uut{rotation1, Vector3::Zero};
    ASSERT_TRUE(areEqualWithinTolerance(rotation1, uut.rotation(), tolerance_));
    uut.rotation() = rotation2;
    ASSERT_FALSE(
        areEqualWithinTolerance(rotation1, uut.rotation(), tolerance_));
...
TEST_F(IsometryTests, Composition) {
  {
    ...
    ASSERT_TRUE(areEqualWithinTolerance(rotation1 * rotation2, uut.rotation(),
                                        tolerance_));
    ASSERT_TRUE(areEqualWithinTolerance(rotation1 * translation2 + translation1,
                                        uut.translation(), tolerance_));
  }
  {
    const Matrix3 rotation1{Vector3{0, 1, 0}, Vector3{0, 0, 1},
                            Vector3{1, 0, 0}};
...
TEST_F(IsometryTests, InverseIsometry) {
  ...
  ASSERT_TRUE(areEqualWithinTolerance(rotation1.inv(), uut.inv().rotation(),
                                      tolerance_));
  ASSERT_TRUE(areEqualWithinTolerance((-1) * rotation1.inv() * translation1,
                                      uut.inv().translation(), tolerance_));
  ASSERT_TRUE(
      areEqualWithinTolerance(Isometry::Identity, uut * uut.inv(), tolerance_));
  ASSERT_TRUE(
      areEqualWithinTolerance(Isometry::Identity, uut.inv() * uut, tolerance_));
}
