...
TEST_F(ModelContainerInterfaceTests, ConstructionTest) {
  const RelativePose3 uut_pose{"frame_id", Position::fromVector(1, 2, 3),
                               Rotation::fromQuaternion(0, 0, 1, 0)};
  ...
  ASSERT_TRUE(RelativePose3::sameRelativePose3(
      uut_pose, uut.pose(), position_tolerance_, angular_tolerance_));

  ASSERT_TRUE(Position::samePosition(
      Position(lbr_corner), uut.containerVolume().lowerRightBackCorner(),
      position_tolerance_));
  ASSERT_TRUE(Position::samePosition(
      Position(ufl_corner), uut.containerVolume().upperLeftFrontCorner(),
      position_tolerance_));
}
