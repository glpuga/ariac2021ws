...
TEST_F(InUseMarkHandlerTests, SharedNullResourceThrows) {
  ASSERT_THROW(
      {
        ResourceHandle<SomeResourse> uut{
            ResourceHandle<SomeResourse>::ResourceSharedPtr{}};
      },
...
TEST_F(InUseMarkHandlerTests, SharedUseCountNeedsToBeOneOnConstruction) {
  ...
  ASSERT_THROW({ ResourceHandle<SomeResourse> uut{resource}; },
               std::invalid_argument);
}
...
TEST_F(InUseMarkHandlerTests, UniqueNullResourceThrows) {
  ASSERT_THROW(
      {
        ResourceHandle<SomeResourse> uut{
            ResourceHandle<SomeResourse>::ResourceUniquePtr{}};
      },
      std::invalid_argument);
}
