...
public:
  ...
  MOCK_METHOD1(findManagedLociByParent, std::vector<ManagedLocusHandle>(const std::string& part_id));
  ...
  MOCK_METHOD3(getModelTrayExclusionZoneHandle,
               std::optional<ExclusionZoneHandle>(const std::string& model_container_name,
                                                  const std::optional<std::string>& additional_model_tray_name_opt,
                                                  const std::chrono::milliseconds& timeout));

  MOCK_METHOD1(getSubmissionTray, std::optional<SubmissionTrayHandle>(const std::string& model_container_name));

  MOCK_CONST_METHOD1(getContainerFrameId, std::string(const std::string& model_container_name));

  MOCK_CONST_METHOD1(getContainerExclusionZoneId, std::string(const std::string& model_container_name));

  MOCK_CONST_METHOD1(getWorkRegionId, WorkRegionId(const ManagedLocusHandle& handle));

  MOCK_CONST_METHOD0(getListOfExclusionZones, const std::vector<ModelTraySharedAccessSpaceDescription>&());
};
