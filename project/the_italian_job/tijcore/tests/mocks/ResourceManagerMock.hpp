...
public:
  ...
  MOCK_METHOD1(getManagedLocusHandleForPose,
               std::optional<ManagedLocusHandle>(const RelativePose3 &pose));

  MOCK_METHOD1(updateSensorData,
               void(const std::vector<ObservedModel> &observed_models));

  MOCK_METHOD3(
      getModelTrayExclusionZoneHandle,
  ...
  MOCK_METHOD1(getSubmissionTray, std::optional<SubmissionTrayHandle>(
                                      const std::string &model_container_name));

  MOCK_CONST_METHOD1(getContainerFrameId,
                     std::string(const std::string &model_container_name));

  MOCK_CONST_METHOD1(getContainerExclusionZoneId,
                     std::string(const std::string &model_container_name));

  MOCK_CONST_METHOD1(getWorkRegionId,
                     WorkRegionId(const ManagedLocusHandle &handle));

  MOCK_CONST_METHOD0(
      getListOfExclusionZones,
