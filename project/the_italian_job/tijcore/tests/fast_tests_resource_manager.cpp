...
public:
  ...
  void buildUnitUnderTest()
  {
    ...
    uut_ = std::make_unique<ResourceManager>(toolbox_, shared_workspaces, std::move(containers_),
                                             std::move(robots_));
  }
};
...
TEST_F(ResourceManagerTests, DISABLED_SensorDataAllocatedTablesIgnoreSensorInput)
{
...

  {
    auto table2_volume_handle =
        uut_->getModelTrayExclusionZoneHandle(table_2_name_, std::nullopt, access_timeout_);
...
TEST_F(ResourceManagerTests, DISABLED_CantAllocateAContainerMoreThanOnce)
{
  ...
  buildUnitUnderTest();
  {
    auto table1_handle_1 =
        uut_->getModelTrayExclusionZoneHandle(table_1_name_, std::nullopt, access_timeout_);
    ASSERT_NE(std::nullopt, table1_handle_1);

    auto table1_handle_2 =
        uut_->getModelTrayExclusionZoneHandle(table_1_name_, std::nullopt, access_timeout_);
    ASSERT_EQ(std::nullopt, table1_handle_2);
  }
  auto table1_handle_3 =
      uut_->getModelTrayExclusionZoneHandle(table_1_name_, std::nullopt, access_timeout_);
...
TEST_F(ResourceManagerTests, DISABLED_FindEmptyLociIgnoresAllocatedContainers)
{
...

  {
    ...
    auto table2_handle =
        uut_->getModelTrayExclusionZoneHandle(table_2_name_, std::nullopt, access_timeout_);
    auto table3_handle =
        uut_->getModelTrayExclusionZoneHandle(table_3_name_, std::nullopt, access_timeout_);
    auto empty_loci = uut_->findEmptyLoci(empty_radius_);
