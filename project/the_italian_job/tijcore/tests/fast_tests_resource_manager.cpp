...
TEST_F(ResourceManagerTests, SensorDataUpdateKnownData) {
...

  {
    ...
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose1, part1_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose2, part2_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose3, part3_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose4, part4_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
  }

  {
    ...
    ASSERT_EQ(1u, part4_handlers.size());
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose4, part1_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose1, part2_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose2, part3_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose3, part4_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
  }

  {
    ...
    ASSERT_EQ(1u, part4_handlers.size());
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose3, part1_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose4, part2_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose1, part3_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose2, part4_handlers[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
  }
}
...
TEST_F(ResourceManagerTests, CreateTargetReturnsKnownLoci) {
  ...
  // allocated
  {
    ...
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose1, handle1->resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose2, handle2->resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose3, handle3->resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose4, handle4->resource()->pose(), position_tolerance_,
        rotation_tolerance_));
  }
...
TEST_F(ResourceManagerTests, GetRobotChoosesTheBestMinimumFit) {
  ...
  action_queue_.queueTestActionQueue([this] {
    ...
    EXPECT_EQ(std::set<WorkRegionId>{},
              pap_robot_handle.value().resource()->supportedRegions());
  });
...
TEST_F(ResourceManagerTests, TestFindManagedLociByParent) {
...

  {
    ...
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose1, parts_in_table1[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
    ASSERT_TRUE(RelativePose3::sameRelativePose3(
        pose3, parts_in_table2[0].resource()->pose(), position_tolerance_,
        rotation_tolerance_));
  }
}
