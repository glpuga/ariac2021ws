...
{
  ASSERT_EQ(RobotTaskOutcome::TASK_SUCCESS, robot_task_outcome::fromString("TASK_SUCCESS"));
  ASSERT_EQ(RobotTaskOutcome::TASK_FAILURE, robot_task_outcome::fromString("TASK_FAILURE"));
  ASSERT_THROW(robot_task_outcome::fromString("bongo"), std::invalid_argument);
}

TEST_F(RobotTaskOutcomeTests, TestToString)
{
  ASSERT_EQ("TASK_SUCCESS", robot_task_outcome::toString(RobotTaskOutcome::TASK_SUCCESS));
  ASSERT_EQ("TASK_FAILURE", robot_task_outcome::toString(RobotTaskOutcome::TASK_FAILURE));
}
