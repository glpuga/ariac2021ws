...
public:
  ...
  const RelativePose3 pose_{"", Position{Vector3{1., 2., 3.}},
                            Rotation::Identity};
...
TEST_F(ManagedLocusTests, EmptySpaceConstruction) {
  ...
  ASSERT_NEAR(pose_.position().vector().x(), uut.pose().position().vector().x(),
              tolerance_);
...
TEST_F(ManagedLocusTests, TransferPartFromHereToThereWorks) {
  ...
  auto is_the_same_pose = [](const RelativePose3 &expected,
                             const RelativePose3 &actual,
                             const double tolerance) {
    auto is_within_tolerance = [](const double a, const double b,
                                  const double tolerance) {
      return ((a - b) < tolerance) && ((b - a) < tolerance);
    };

    return (expected.frameId() == actual.frameId()) &&
           is_within_tolerance(expected.position().vector().x(),
                               actual.position().vector().x(), tolerance) &&
           is_within_tolerance(expected.position().vector().y(),
                               actual.position().vector().y(), tolerance) &&
           is_within_tolerance(expected.position().vector().z(),
                               actual.position().vector().z(), tolerance) &&
  ...
  const RelativePose3 src_pose{"", Position{Vector3{1., 2., 3.}},
                               Rotation::Identity};
  const RelativePose3 dst_pose{"", Position{Vector3{10., 20., 30.}},
                               Rotation::Identity};

  {
    // From occupied to emtpy, should work just fine
    auto src =
        ManagedLocus::CreateOccupiedSpace(src_parent, src_pose, part_id_, true);
...

  {
    ...
    auto dst =
        ManagedLocus::CreateOccupiedSpace(dst_parent, dst_pose, part_id_, true);
    ASSERT_TRUE(is_empty_space(src));
    ASSERT_TRUE(is_the_same_pose(src_pose, src.pose(), tolerance_));
    ASSERT_EQ(src_parent, src.parentName());
    ASSERT_TRUE(is_occupied_space(dst, part_id_, true));
    ...
    ASSERT_THROW({ ManagedLocus::TransferPartFromHereToThere(src, dst); },
                 std::logic_error);
  }
