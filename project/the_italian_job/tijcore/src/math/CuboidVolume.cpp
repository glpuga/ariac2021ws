...
CuboidVolume::CuboidVolume(const Vector3 &lower_right_back,
                           const Vector3 &upper_left_front)
...
bool CuboidVolume::contains(const Vector3 &point) const {
  auto a_less_than_x_less_than_b = [](const double a, const double x,
                                      const double b) {
    return (a <= x) && (x <= b);
  };
  return a_less_than_x_less_than_b(lower_right_back_.x(), point.x(),
                                   upper_left_front_.x()) &&
         a_less_than_x_less_than_b(lower_right_back_.y(), point.y(),
                                   upper_left_front_.y()) &&
         a_less_than_x_less_than_b(lower_right_back_.z(), point.z(),
                                   upper_left_front_.z());
}
