...
PartId::PartId(const std::string &coded_part_str) {
  ...
  if ((tokens.size() != 3) || tokens[0] != coded_part_string_prefix_ ||
    ...
    throw std::invalid_argument{coded_part_str +
                                " is not a valid coded part descriptor"};
  }
...
PartId::PartId(const PartTypeId &type, const PartColorId &color)
...
std::string PartId::codedString() const {
  ...
  return utils::string::joinStringsWithSeparator({coded_part_string_prefix_,
                                                  part_type::toString(type_),
                                                  part_color::toString(color_)},
                                                 coded_part_string_separator_);
}
...
bool PartId::operator!=(const PartId &rhs) const { return !(*this == rhs); }
