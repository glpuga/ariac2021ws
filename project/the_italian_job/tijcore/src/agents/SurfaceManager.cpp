...
SurfaceManager::SurfaceManager(const double x0, const double y0,
                               const double width, const double height)
    : x0_{x0}, y0_{y0}, width_{width}, height_{height} {}

void SurfaceManager::markAsOccupied(const double x, const double y,
                                    const double radius) {
  regions_in_use_.push_back(RegionInUse{x, y, radius});
}

bool SurfaceManager::regionIsAvailable(const double x, const double y,
...
SurfaceManager::findFreeRegion(const double free_radius) const {
  ...
  if (!regions_in_use_.empty()) {
    ...
    opt_best_candidate =
        RegionInUse{x0_ + free_radius, y0_ + free_radius, free_radius};
  }

  if (!opt_best_candidate ||
      !regionIsWithinSurface(opt_best_candidate.value()) ||
      !regionIsAvailable(opt_best_candidate.value())) {
    return std::nullopt;
  }

  return std::make_tuple(opt_best_candidate.value().x,
                         opt_best_candidate.value().y);
}

std::optional<double>
SurfaceManager::minimumDistanceToOthers(const RegionInUse &candidate) const {
  if (!regionIsWithinSurface(candidate)) {
    /* LOG WARNING a region outside of the surface is being asked about */
    return std::nullopt;
  }
...
bool SurfaceManager::regionsOverlap(const RegionInUse &region_a,
                                    const RegionInUse &region_b) {
  return (distanceBetweenRegions(region_a, region_b) <
          (region_a.radius + region_b.radius));
}
