...
void factoryLoaderMethod(BT::BehaviorTreeFactory& factory)
{
  ...
  factory.registerNodeType<SwapSourceAndDestinationLociNode>("SwapSourceAndDestinationLociNode");
};
...
{
  auto bt_task_parameters = std::make_shared<BTTaskData>();
  bt_task_parameters->src_locus = std::move(source_locus);
  bt_task_parameters->primary_robot = std::move(robot);

  auto blackboard = BT::Blackboard::create();
  blackboard->set<BTTaskData::SharedPtr>(  // NOLINT(build/include_what_you_use)
