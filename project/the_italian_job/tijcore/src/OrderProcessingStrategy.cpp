    ...
    const std::set<StationId>& assemblies_in_use) const
{
  ...
  for (const auto& product : products)
  {
    ...
    auto handle_opt = resource_manager_->getManagedLocusHandleForPose(desired_pose);
    ...
    if (locus.isEmpty())
    {
      ...
      if (handle.allocationCount() <= 2)
      {
        ...
        missing_parts.emplace_back(std::make_pair(std::move(handle), desired_part_id));
      }
      else
      {
  ...
  {
    auto parts_to_remove = resource_manager_->findManagedLociByParent(target_container_name);
  ...
  if (unwanted_parts.size())
  {
    ...
    for (auto& part : unwanted_parts)
    {
      ...
      while (empty_loci.size())
      {
        ...
        work_regions.emplace(resource_manager_->getWorkRegionId(closest_empty_spot));

        auto robot_handle_opt = resource_manager_->getPickAndPlaceRobotHandle(work_regions);
  ...
  if (missing_parts.size())
  {
    for (auto& [missing_part_locus, part_id] : missing_parts)
    {
      ...
      // sort by distance (larger distance first, to remove efficiently the
      // nearest spot from the list)
      std::sort(potential_sources.begin(), potential_sources.end(), sort_farthest_first);
      ...
      else
      {
        ...
        const auto missing_part_region = resource_manager_->getWorkRegionId(missing_part_locus);
        const auto selected_part_region = resource_manager_->getWorkRegionId(selected_source_part);
        std::set<WorkRegionId> work_regions;
        ...
        if (robot_handle_opt)
        {
          ...
          if (!special_empty_loci.empty() && (conveyor_belt_piece || part_needs_flipping))
          {
            ...
            std::sort(special_empty_loci.begin(), special_empty_loci.end(), sort_farthest_first);
            ...
            {
              WARNING(
                  "Creating a PickAndPlaceTask for {} to remove part "
            ...
            {
              WARNING(
                  "Creating a PickAndTwist for {} for a part at {}, "
                  "manipulating at {}",
                  robot_handle_opt->resource()->name(), selected_source_part.resource()->pose(),
                  closest_empty_spot.resource()->pose());
      ...
      (products.size() == parts_in_place.size() + unavailable_part_count))
  {
    ...
    auto tray_handle = resource_manager_->getSubmissionTray(target_container_name);
