...
TaskMaster::TaskMaster(
  ...
  if (!order_strategy_) {
    order_strategy_ = std::make_unique<OrderProcessingStrategy>(
        resource_manager, robot_task_factory, toolbox_);
  } else {
  }
}
...
void TaskMaster::registerOrder(const Order &order) {
  ...
  if (orders_.count(key)) {
    if (!order.order_id.isUpdate()) {
      WARNING("A new order was received with the same id as a previous one "
...
std::vector<RobotTaskInterface::Ptr> TaskMaster::run() {
  ...
  for (const auto &key : priority_sorted_keys) {
    ...
    order_strategy_->getAgvsAndStationsInUse(order, agsv_in_use,
                                             stations_in_use);
  }
  ...
  for (const auto &key : priority_sorted_keys) {
    auto &order = orders_.at(key);
    order_strategy_->disambiguateOrderEndpoints(order, agsv_in_use,
                                                stations_in_use);
    auto order_tasks =
        order_strategy_->processOrder(order, agsv_in_use, stations_in_use);

    std::move(order_tasks.begin(), order_tasks.end(),
