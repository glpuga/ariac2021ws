...
RobotTaskInterface::Ptr RobotTaskFactory::getRemoveBrokenPartTask(
  ...
  return std::make_unique<RemoveBrokenPartTask>(
      resource_manager_, toolbox_, std::move(source_locus), std::move(robot));
}
...
RobotTaskInterface::Ptr RobotTaskFactory::getSubmitKittingShipmentTask(
  ...
  return std::make_unique<SubmitKittingShipmentTask>(
      toolbox_, std::move(tray), destination_station, shipment_type);
}
...
RobotTaskInterface::Ptr RobotTaskFactory::getSubmitAssemblyShipmentTask(
  ...
  return std::make_unique<SubmitAssemblyShipmentTask>(toolbox_, std::move(tray),
                                                      shipment_type);
}
