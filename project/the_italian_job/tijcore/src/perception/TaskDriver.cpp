...
TaskDriver::TaskDriver(
    TaskMasterInterface::Ptr &&task_master,
    ...
    : processing_interval_{processing_interval},
      task_master_{std::move(task_master)}, task_group_runner_{std::move(
                                                task_group_runner)},
      resource_manager_{resource_manager},
      perception_input_{perception_input}, toolbox_{toolbox} {
...
void TaskDriver::perceptionCallback() {
  ...
  INFO("Updating sensor information with {} observed models",
       observed_models.size());
...
void TaskDriver::processingCallback() {
  ...
  if (!robot_actions_to_execute.empty()) {
    INFO("Got {} new tasks, initiating execution",
         robot_actions_to_execute.size());
