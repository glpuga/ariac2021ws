...
ManagedLocus
ManagedLocus::CreateOccupiedSpace(const std::string &parent_container,
                                  const RelativePose3 &pose,
...
void ManagedLocus::TransferPartFromHereToThere(ManagedLocus &here,
                                               ManagedLocus &there) {
  if (!here.isModel()) {
...
ManagedLocus::ManagedLocus(const std::string &parent_container,
                           const RelativePose3 &pose)
    : parent_container_{parent_container}, pose_{pose},
      part_id_{PartId(PartTypeId::pump, PartColorId::red)}, is_model_{false},
      is_broken_{false} {}

ManagedLocus::ManagedLocus(const std::string &parent_container,
                           const RelativePose3 &pose, const PartId &part_id,
                           const bool broken)
    : parent_container_{parent_container}, pose_{pose}, part_id_{part_id},
      is_model_{true}, is_broken_{broken} {}

bool ManagedLocus::isEmpty() const { return !is_model_; }

bool ManagedLocus::isModel() const { return is_model_; }
