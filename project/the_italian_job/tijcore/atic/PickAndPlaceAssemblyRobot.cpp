...
std::set<tijcore::WorkRegionId> PickAndPlaceAssemblyRobot::supportedRegions() const
{
  // auto health_status = robot_actuator_->getRobotHealthStatus();
  ...
  return { tijcore::WorkRegionId::assembly, tijcore::WorkRegionId::kitting_agvs,
           tijcore::WorkRegionId::kitting_near_bins, tijcore::WorkRegionId::kitting_far_bins };
}
    ...
    std::vector<double>& joint_states) const
{
  ...
  tijcore::RelativePose3 current_pose_estimation{
    long_rail_frame_id_, tijcore::Position::fromVector(joint_states[1], joint_states[0], 0.0), {}
  };
    ...
    const std::vector<tijcore::RelativePose3>& pose_hints) const
{
  ...
  auto shortest_distance_to_reference_sorter = [this, &reference = target_in_world](
                                                   const tijcore::RelativePose3& lhs,
