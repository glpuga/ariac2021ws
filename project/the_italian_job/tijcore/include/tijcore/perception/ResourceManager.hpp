...
public:
  ...
  std::vector<ManagedLocusHandle>
  findManagedLociByParent(const std::string &parent_name) override;
  ...
  getManagedLocusHandleForPose(const RelativePose3 &pose) override;

  void
  updateSensorData(const std::vector<ObservedModel> &observed_models) override;
  ...
  std::string
  getContainerFrameId(const std::string &model_container_name) const override;

  std::string getContainerExclusionZoneId(
      const std::string &model_container_name) const override;
...
private:
  ...
  const double position_tolerance_{
      0.024}; // TODO(glpuga) analyze what value should be used for this
  const double default_occupancy_radius_{
      0.02}; // TODO(glpuga) this fixed value should be replaced by the
             // footprint of a given part, or at least be a function of part
             // size.

  const std::vector<ModelTraySharedAccessSpaceDescription>
      shared_workspace_descriptors_;
  ...
  const ModelContainerHandle *
  findLociContainer(const ManagedLocus &locus) const;

  void clearEmptyLoci();

  Pose3 transformPoseToContainerLocalPose(
      const RelativePose3 &relative_pose,
      const std::string &container_frame_id) const;

  std::tuple<double, double, double>
  poseToOccupancy(const RelativePose3 &relative_pose,
                  const std::string &container_frame_id) const;

  SurfaceManager
  buildContainerSurfaceManager(const std::string &parent_name) const;

  SurfaceManager volumeToSurface(const CuboidVolume &v) const;

  bool locusWithinVolume(const ManagedLocus &locus,
