...
{
  ctx.advance_to(
      visit_format_arg(arg_formatter<OutputIt, typename Context::char_type>(ctx, &parse_ctx), ctx.arg(arg_id)));
}
  ...
  for (auto part_it = std::begin(parts); part_it != std::end(parts); ++part_it)
  {
    ...
    switch (part.part_kind)
    {
      ...
      case format_part_t::kind::replacement:
      {
        ...
        advance_to(parse_ctx, part.arg_id_end);
        ctx.advance_to(
            visit_format_arg(arg_formatter<OutputIt, typename Context::char_type>(ctx, nullptr, &specs), arg));
        break;
      }
    }
  }
...
private:
  ...
  template <typename Context, typename OutputIt, typename CompiledFormat>
  friend auto cf::vformat_to(OutputIt out, CompiledFormat& cf, basic_format_args<Context> args) ->
...
constexpr auto compile_format_string(S format_str)
{
  ...
  if constexpr (str[POS] == '{')
  {
    ...
    else if constexpr (str[POS + 1] == ':')
    {
      ...
      return parse_tail<Args, result.end, result.next_arg_id>(spec_field<char_type, type, ID>{ result.fmt },
                                                              format_str);
    }
    else
    {
...
template <typename... Args, typename S,
          FMT_ENABLE_IF(is_compile_string<S>::value || detail::is_compiled_string<S>::value)>
...
template <typename OutputIt, typename CompiledFormat, typename... Args>
auto format_to_n(OutputIt out, size_t n, const CompiledFormat& cf, const Args&... args) ->
    typename std::enable_if<detail::is_output_iterator<OutputIt, typename CompiledFormat::char_type>::value &&
