...
TIJChallenger::TIJChallenger() {
  ...
  auto resource_manager = std::make_shared<tijcore::ResourceManager>(
      toolbox_, config_->getListOfSharedAccessSpaceDescriptions(),
  ...
  auto task_master = std::make_unique<tijcore::TaskMaster>(
      resource_manager,
      std::make_unique<tijcore::RobotTaskFactory>(resource_manager, toolbox_),
      toolbox_);

  INFO(" - Creating TaskDriver");
  task_driver_ = std::make_unique<tijcore::TaskDriver>(
      std::move(task_master), std::make_unique<tijcore::RobotTaskGroupRunner>(),
...
tijcore::ModelPerceptionInterface::Ptr
TIJChallenger::createModelPerceptionMixer() const {
  ...
  for (const auto &item : config_->getListOfLogicalCameras()) {
    ...
    cameras.emplace_back(
        std::make_unique<tijros::LogicalCameraModelPerception>(nh_, item.name));
  }
  ...
  for (const auto &item : config_->getListOfQualityControlSensors()) {
    INFO("   - {} @ {}", item.name, item.frame_id);
    cameras.emplace_back(
        std::make_unique<tijros::QualityControlSensorModelPerception>(
            nh_, item.name));
  }
...
tijcore::Toolbox::SharedPtr TIJChallenger::createToolbox() const {
  ...
  contents.frame_transformer_instance =
      std::make_shared<tijros::ROSFrameTransformer>();
  contents.robot_actuator_instance =
      std::make_shared<tijros::ROSRobotActuators>(nh_);
  contents.process_manager_instance =
      std::make_shared<tijros::ROSProcessManagement>(nh_);
...
TIJChallenger::createModelContainers(
    const tijcore::Toolbox::SharedPtr &toolbox_) const {
  ...
  for (const auto &item : config_->getListOfAssemblyStations()) {
    ...
    containers.emplace_back(
        std::make_unique<tijcore::AssemblyStationModelContainer>(
            item.name, item.frame_id, item.shared_access_space_id, toolbox_));
  }
...
TIJChallenger::createPickAndPlaceRobots(
    const tijcore::Toolbox::SharedPtr &toolbox_) const {
  INFO(" - Creating robot puppeteers");
  std::vector<tijcore::PickAndPlaceRobotInterface::Ptr> robots;
  robots.emplace_back(
      std::make_unique<tijros::PickAndPlaceKittingRobot>(toolbox_));
  robots.emplace_back(
      std::make_unique<tijros::PickAndPlaceAssemblyRobot>(toolbox_));
  return robots;
}
