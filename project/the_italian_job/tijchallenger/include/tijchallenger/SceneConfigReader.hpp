...
public:
  ...
  const std::vector<DeviceData> &
  getListOfQualityControlSensors() const override;
  ...
  getListOfGantryPlanningHints() const override;

  const std::vector<tijcore::RelativePose3> &
  getListOfSafeWaitingSpotHints() const override;
};
