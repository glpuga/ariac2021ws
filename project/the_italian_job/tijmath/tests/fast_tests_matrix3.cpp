...
{
  const Matrix3 uut{ { 1, 2, 3 }, { 4, 5, 6 }, { 7, 8, 9 } };
  ASSERT_TRUE(areEqualWithinTolerance(uut.row(0), Vector3{ 1, 2, 3 }, tolerance_));
  ASSERT_TRUE(areEqualWithinTolerance(uut.row(1), Vector3{ 4, 5, 6 }, tolerance_));
  ASSERT_TRUE(areEqualWithinTolerance(uut.row(2), Vector3{ 7, 8, 9 }, tolerance_));
...
{
  const Matrix3 uut{ { 1, 2, 3 }, { 4, 5, 6 }, { 7, 8, 9 } };
  ASSERT_TRUE(areEqualWithinTolerance(uut.col(0), Vector3{ 1, 4, 7 }, tolerance_));
  ASSERT_TRUE(areEqualWithinTolerance(uut.col(1), Vector3{ 2, 5, 8 }, tolerance_));
  ASSERT_TRUE(areEqualWithinTolerance(uut.col(2), Vector3{ 3, 6, 9 }, tolerance_));
...
{
  {
    ...
    EXPECT_TRUE(areEqualWithinTolerance(Matrix3::Identity, uut.inv() * uut, tolerance_));
  }
  {
    const Matrix3 uut{ { 1, 0, 0 }, { 0, 2, 0 }, { 0, 0, 4 } };
    const Matrix3 expected{ Vector3{ 1, 0, 0 }, Vector3{ 0, 0.5, 0 }, Vector3{ 0, 0, 0.25 } };
    EXPECT_TRUE(areEqualWithinTolerance(expected, uut.inv(), tolerance_));
    EXPECT_TRUE(areEqualWithinTolerance(Matrix3::Identity, uut * uut.inv(), tolerance_));
    EXPECT_TRUE(areEqualWithinTolerance(Matrix3::Identity, uut.inv() * uut, tolerance_));
  }
