...
RelativePose3 ROSFrameTransformer::transformPoseToFrame(
  ...
  try {
    ...
    tf2::doTransform(ros_input_pose, ros_output_pose,
                     tf_buffer_.lookupTransform(new_frame_id,
                                                relative_pose.frameId(),
    ...
    ERROR("Error while tranforming between frames {} and {}: {}",
          relative_pose.frameId(), new_frame_id, ex.what());
    throw std::runtime_error{"Failed ROS frame transformation"};
  }

  auto output_frame_id = ros_output_pose.header.frame_id;
  auto local_output_pose =
      utils::convertGeoPoseToCorePose(ros_output_pose.pose);
  tijcore::RelativePose3 output_pose{output_frame_id, local_output_pose};

  DEBUG("Transformed pose {} to frame {}, resulting in the pose {}",
        relative_pose, new_frame_id, output_pose);
