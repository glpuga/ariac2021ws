...
ROSProcessManagement::ROSProcessManagement(const ros::NodeHandle &nh)
    : nh_{nh} {
  competition_state_sub_ =
      nh_.subscribe(competition_state_topic, default_queue_len,
                    &ROSProcessManagement::competitionStateCallback, this);

  orders_sub_ = nh_.subscribe(orders_topic, default_queue_len,
                              &ROSProcessManagement::ordersCallback, this);

  agv1_state_sub_ =
      nh_.subscribe(agv1_state_topic, default_queue_len,
                    &ROSProcessManagement::agv1StateCallback, this);
  agv2_state_sub_ =
      nh_.subscribe(agv2_state_topic, default_queue_len,
                    &ROSProcessManagement::agv2StateCallback, this);
  agv3_state_sub_ =
      nh_.subscribe(agv3_state_topic, default_queue_len,
                    &ROSProcessManagement::agv3StateCallback, this);
  agv4_state_sub_ =
      nh_.subscribe(agv4_state_topic, default_queue_len,
                    &ROSProcessManagement::agv4StateCallback, this);

  agv1_station_sub_ =
      nh_.subscribe(agv1_station_topic, default_queue_len,
                    &ROSProcessManagement::agv1StationCallback, this);
  agv2_station_sub_ =
      nh_.subscribe(agv2_station_topic, default_queue_len,
                    &ROSProcessManagement::agv2StationCallback, this);
  agv3_station_sub_ =
      nh_.subscribe(agv3_station_topic, default_queue_len,
                    &ROSProcessManagement::agv3StationCallback, this);
  agv4_station_sub_ =
      nh_.subscribe(agv4_station_topic, default_queue_len,
                    &ROSProcessManagement::agv4StationCallback, this);
}

ProcessManagementInterface::ErrorWithReason
ROSProcessManagement::startCompetition() const {
...
ProcessManagementInterface::ErrorWithReason
ROSProcessManagement::endCompetition() const {
  std::lock_guard<std::mutex> lock{mutex_};
  std_srvs::Trigger msg;
...
ProcessManagementInterface::ErrorWithReason
ROSProcessManagement::submitAgvToAssemblyStation(
  ...
  msg.request.assembly_station_name =
  ...
  if (!msg.response.success) {
    ERROR("Failed to submit shipment {}: {}", shipment_type,
          msg.response.message);
  } else {
    INFO("Submitted shipment {} (msg: {})", shipment_type,
         msg.response.message);
  }
  return std::make_pair(msg.response.success, msg.response.message);
}
...
ProcessManagementInterface::ErrorWithReason
ROSProcessManagement::submitAssemblyStation(
  ...
  auto message =
      "inspection_result = " + std::to_string(msg.response.inspection_result);
...
void ROSProcessManagement::competitionStateCallback(
...
void ROSProcessManagement::ordersCallback(nist_gear::Order::ConstPtr msg) {
  ...
  auto convert_ros_product_to_core_product =
      [](const nist_gear::Product &ros_msg_product_description,
         const std::string &tray_frame_id) {
        return tijcore::ProductRequest{
            tijcore::PartId(ros_msg_product_description.type),
            tijcore::RelativePose3{tray_frame_id,
                                   utils::convertGeoPoseToCorePose(
  ...
  for (const auto &input_kitting_shipment : msg->kitting_shipments) {
    ...
    output_kitting_shipment.agv_id =
        tijcore::agv::fromString(input_kitting_shipment.agv_id);
    output_kitting_shipment.station_id =
        tijcore::station_id::fromString(input_kitting_shipment.station_id);
    output_kitting_shipment.shipment_type =
        input_kitting_shipment.shipment_type;

    auto kit_tray_frame_id =
        utils::convertAgvIdToKitTrayFrameId(output_kitting_shipment.agv_id);

    for (const auto &input_product : input_kitting_shipment.products) {
      output_kitting_shipment.products.push_back(
          convert_ros_product_to_core_product(input_product,
                                              kit_tray_frame_id));
    }
  ...
  for (const auto &input_assembly_shipment : msg->assembly_shipments) {
    ...
    output_assembly_shipment.shipment_type =
        input_assembly_shipment.shipment_type;

    auto briefcase_frame_id = utils::convertAgvIdToBriefcaseFrameId(
        output_assembly_shipment.station_id);
