...
LogicalCameraModelPerception::LogicalCameraModelPerception(
    const ros::NodeHandle &nh, const std::string &logical_sensor_name)
    : nh_{nh}, logical_sensor_name_{logical_sensor_name}, timer_{[this] {
        timerCallback();
      }} {
  const std::string topic_id{topic_prefix_ + logical_sensor_name_};
  camera_sub_ =
      nh_.subscribe(topic_id, default_queue_len_,
                    &LogicalCameraModelPerception::cameraCallback, this);
  timer_.start(timer_interval_);
}

std::vector<tijcore::ObservedModel>
LogicalCameraModelPerception::getObservedModels() const {
  std::lock_guard<std::mutex> lock{mutex_};
  return models_;
}

void LogicalCameraModelPerception::cameraCallback(
  ...
  for (const auto &ros_model : msg->models) {
    ...
    const auto relative_core_pose =
        tijcore::RelativePose3{logical_sensor_name_ + "_frame",
...
void LogicalCameraModelPerception::timerCallback() {
  std::lock_guard<std::mutex> lock{mutex_};
  if (!update_received_) {
    WARNING("No updates on {}, possibly going through a sensor blackout",
            logical_sensor_name_);
    models_.clear();
  }
