...
std::string convertAgvIdToBriefcaseFrameId(const tijcore::StationId& station_id)
{
  ...
  switch (station_id)
  {
    ...
    case tijcore::StationId::ks4:
      throw std::invalid_argument{ "a target station is in a kitting station is not valid" };
      break;
    case tijcore::StationId::any:
      throw std::invalid_argument{ "station_id valued any cannot be given a frame id" };
      break;
  }
