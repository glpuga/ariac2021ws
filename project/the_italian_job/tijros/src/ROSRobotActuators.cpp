...
constexpr char gantry_arm_gripper_state_topic[] =
    "/ariac/gantry/arm/gripper/state";
constexpr char kitting_arm_gripper_state_topic[] =
    "/ariac/kitting/arm/gripper/state";

constexpr char gantry_arm_gripper_control_service[] =
    "/ariac/gantry/arm/gripper/control";
constexpr char kitting_arm_gripper_control_service[] =
    "/ariac/kitting/arm/gripper/control";

constexpr char gantry_tray_lock_models_service[] =
    "/ariac/gantry_tray/lock_models";
constexpr char gantry_tray_unlock_models_service[] =
    "/ariac/gantry_tray/unlock_models";
...
ROSRobotActuators::ROSRobotActuators(const ros::NodeHandle nh) : nh_{nh} {
  conveyor_state_sub_ =
      nh_.subscribe(converyor_state_topic, default_queue_len,
  ...
  robot_health_sub_ =
      nh_.subscribe(robot_health_topic, default_queue_len,
                    &ROSRobotActuators::robotHealthCallback, this);
}

RobotActuatorsInterface::ConveyorState
ROSRobotActuators::getConveyorState() const {
  std::lock_guard<std::mutex> lock{mutex_};
  return latest_conveyor_state_data_;
}

RobotActuatorsInterface::VacuumGripperState
ROSRobotActuators::getGantryGripperState() const {
  std::lock_guard<std::mutex> lock{mutex_};
...
RobotActuatorsInterface::VacuumGripperState
ROSRobotActuators::getKittingGripperState() const {
  std::lock_guard<std::mutex> lock{mutex_};
...
bool ROSRobotActuators::setGantryTrayLockState(const bool lock_state) const {
  ...
  if (!msg.response.success) {
    ERROR("Failed to set the gantry lock state to {}",
          lock_state ? "ON" : "OFF");
    return false;
  }
...
RobotActuatorsInterface::RobotHealthStatus
ROSRobotActuators::getRobotHealthStatus() const {
  std::lock_guard<std::mutex> lock{mutex_};
  return latest_robot_health_data_;
}

void ROSRobotActuators::conveyorStateCallback(
    nist_gear::ConveyorBeltState::ConstPtr msg) {
  std::lock_guard<std::mutex> lock{mutex_};
  latest_conveyor_state_data_.power = msg->power;
  latest_conveyor_state_data_.enabled = msg->enabled;
}

void ROSRobotActuators::gantryArmGripperStateCallback(
    nist_gear::VacuumGripperState::ConstPtr msg) {
  std::lock_guard<std::mutex> lock{mutex_};
  latest_gantry_arm_gripper_state_data_.enabled = msg->enabled;
  latest_gantry_arm_gripper_state_data_.attached = msg->attached;
}

void ROSRobotActuators::kittingArmGripperStateCallback(
    nist_gear::VacuumGripperState::ConstPtr msg) {
  std::lock_guard<std::mutex> lock{mutex_};
...
void ROSRobotActuators::robotHealthCallback(
    nist_gear::RobotHealth::ConstPtr msg) {
  std::lock_guard<std::mutex> lock{mutex_};
  latest_robot_health_data_.kitting_robot_enabled = msg->kitting_robot_enabled;
  latest_robot_health_data_.assembly_robot_enabled =
      msg->assembly_robot_enabled;
}
